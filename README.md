...
- **Dispatch** (footer): type a prompt, pick Chat vs Code, tick one or more agents, and open them as real Chrome tabs with the prompt pre-filled (not sent). Claude Code uses the official `claude.ai/code?prompt=&repositories=` params. Codex (`chatgpt.com/codex`), Cursor Cloud Agents (`cursor.com/agents`), Gemini (`gemini.google.com/app`), and Grok (`grok.com`) get a content-script fill so nothing is auto-submitted — Codex's documented `prompt=` prefill is ChatGPT chat / the desktop `codex://` scheme, not the cloud page. Grok Bot stays on the quota dashboard only: it is a desktop/iOS app with no public web composer. A board in the panel lists the tabs so you can jump back without leaving Token Police. Auto-dispatch skips anyone under 15% remaining, and anyone with no quota data yet.
