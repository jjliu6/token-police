...
try {
  ...
  for (const [name, html] of [['/', root], ['/en/', en], ['/fr/', fr], ['/zh/', zh]]) {
    ...
    assert.ok(html.includes('hreflang="fr" href="https://token-police.philosophie.ai/fr/"'), `${name}: hreflang fr`);
  ...
  assert.ok(en.includes('href="/fr/" hreflang="fr" lang="fr" title="Passer en français">FR</a>'), 'en page links to /fr/');
  ...
  assert.ok(root.includes("location.replace('/fr/'"), 'root redirects French readers');
  ...
  assert.ok(zh.includes('href="/fr/" hreflang="fr" lang="fr" title="Passer en français">FR</a>'), 'zh page links to /fr/');
  ...
  // French page.
  assert.ok(fr.includes('<html lang="fr">'));
  assert.ok(fr.includes('<link rel="canonical" href="https://token-police.philosophie.ai/fr/">'));
  assert.ok(fr.includes('<title>Token Police'));
  assert.ok(fr.includes('<meta name="description" content="Extension Chrome gratuite'), 'fr meta description translated');
  assert.ok(fr.includes('<meta property="og:locale" content="fr_FR">'));
  assert.ok(fr.includes('href="/en/" hreflang="en" lang="en" title="Switch to English">EN</a>'), 'fr page links to /en/');
  assert.ok(fr.includes('href="/zh/" hreflang="zh-CN" lang="zh-CN" title="切换到中文">中文</a>'), 'fr page links to /zh/');
  assert.ok(fr.includes('src="/shot-panel-en.png"') && fr.includes('src="/install-demo-en.webm"'), 'fr reuses English screenshots');
  assert.ok(fr.includes('<h2>Prêt ? C’est gratuit.</h2>') || fr.includes('<h2>Prêt ? C&rsquo;est gratuit.</h2>') || fr.includes("Prêt ? C’est gratuit."), 'fr body text translated');
  ...
  for (const [name, html] of [['/en/', en], ['/fr/', fr], ['/zh/', zh], ['/', root]]) {
    ...
    assert.ok(!html.includes('data-network="weibo"') && !html.includes('service.weibo.com'), `${name}: no Weibo integration`);
    assert.ok(!html.includes('id="share-native"') && !html.includes('navigator.share'), `${name}: no unrestricted native share sheet`);
  ...
  assert.ok(fr.includes('Copier le lien'), 'fr copy label');
  assert.ok(fr.includes('title="Partager cette page"'), 'fr header share title');
  assert.ok(fr.includes('quota restant de Claude Code'), 'fr share text');
  assert.ok(!fr.includes('Like it? Spread the word.'), 'fr has no English share prompt');
  assert.ok(fr.includes('>Copier le lien</span>') && !fr.includes('>Copy link</span>'), 'fr copy button is translated');
