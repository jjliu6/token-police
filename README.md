...
- **Dispatch** (footer): type a prompt, pick Chat vs Code, tick one or more agents, and open them as real Chrome tabs with the prompt pre-filled (not sent). Claude Code / Codex use official URL params; Cursor / Gemini / Grok get a content-script fill so nothing is auto-submitted. A board in the panel lists the tabs so you can jump back without leaving Token Police. Auto-dispatch skips anyone under 15% remaining, and anyone with no quota data yet.
