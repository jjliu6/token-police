...
- **Cross-check** (footer): from the **current tab** (ChatGPT, Claude, Grok, Gemini, Codex, Cursor Agents), hand the transcript to a second AI of the **same kind** — chat sources only offer chat reviewers, coding sources only offer coding reviewers. Two explicit reviewer modes: **System recommend** (default — the matching-kind reviewer with the most remaining quota) and **Pick myself**. Chat and coding each have their own default review instruction. **Prefill by default** — an optional **Auto-send** toggle (off by default, Chat targets only, same `canAutoSend()` gate as Dispatch) will actually submit and use quota. Coding reviewers stay prefill-only. Long sessions download `conversation.md` instead of stuffing the composer. It does not call a model API or render the reviewer's findings.
...
- `content.js` — content script that reads usage numbers from each product page, extracts a Cross-check transcript, and fills a prompt on dispatch
- `conversation-adapters.js` — per-host transcript extractors (ChatGPT, Claude, Grok, Gemini, Codex, Cursor)
- `review.js` — Cross-check prompt assembly (chat vs code defaults), GitHub-link detection, `conversation.md` fallback, same-kind highest-quota reviewer recommend
