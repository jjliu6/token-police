...
- **Cross-check** (footer): from a ChatGPT conversation, pick a second AI and Token Police prefills that tab with a review instruction plus the current transcript. Prefill only — never auto-sent. Long sessions download `conversation.md` instead of stuffing the composer. v1 reads ChatGPT chats only; it does not call a model API or render the reviewer's findings.
